<!DOCTYPE html>
<html>

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, shrink-to-fit=no">
    <title>PPGES - Subjects</title>
    <link rel="stylesheet" href="../assets/bootstrap/css/bootstrap.min.css">
    <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Montserrat:400,400i,700,700i,600,600i">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/baguettebox.js/1.10.0/baguetteBox.min.css">
    <link rel="stylesheet" href="../assets/css/smoothproducts.css">
</head>

<body>
        <nav class="navbar navbar-light navbar-expand-xl fixed-top bg-white clean-navbar" style="padding-right: 20px;padding-left: 20px;padding-top: 0px;padding-bottom: 0px;">
                <div class="container-fluid"><button data-toggle="collapse" class="navbar-toggler" data-target="#navcol-1"><span class="sr-only">Toggle navigation</span><span class="navbar-toggler-icon"></span></button><img class="img-fluid" src="../assets/img/unipampa.png" width="120px" style="margin: 0px;margin-left: 40px;">
                    <div
                        class="collapse navbar-collapse" id="navcol-1">
                        <ul class="nav navbar-nav ml-auto">
                            <li class="nav-item" role="presentation" style="padding-right: 10px;"><a class="nav-link active" href="indexEnglish.php">Presentation</a></li>
                            <li class="nav-item text-uppercase my-auto" role="presentation" style="padding-right: 0px;">
                                <div class="nav-item dropdown align-content-start"><a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false" href="#" style="color: rgba(0,0,0,0.9);font-size: 12.8px;font-weight: 600;">
                                        students and teachers</a>
                                    <div class="dropdown-menu" role="menu" style="font-size: 12px;"><a class="dropdown-item" role="presentation" href="coordenadoresEnglish.php">coordination</a><a class="dropdown-item" role="presentation" href="orientadoresEnglish.php">guiding teachers</a><a class="dropdown-item" role="presentation" href="alunosEnglish.php">Students</a></div>
                                </div>
                            </li>
                            <li class="nav-item" role="presentation" style="padding-right: 10px;"><a class="nav-link active" href="eventosEnglish.php">Events</a></li>
                            <li class="nav-item" role="presentation" style="padding-right: 10px;"><a class="nav-link active" href="seminariosEnglish.php">Seminars and Lectures</a></li>
                            <li class="nav-item" role="presentation" style="padding-right: 10px;"><a class="nav-link active" href="disciplinasEnglish.php">
                                    Master's Subjects</a></li>
                            <li class="nav-item" role="presentation" style="padding-right: 0px;"><a class="nav-link active" href="informesEnglish.php">Reports</a></li>
                            <li style="padding-right: 0px;" class="nav-item" role="presentation"><a href="../disciplinas.php" class="nav-link active" style="font-weight: 300; font-size: 12px; vertical-align: -webkit-baseline-middle; text-transform: none;">Versão em Português</a></li>
        
                        </ul>
                </div>
                </div>
            </nav>
            <main class="page landing-page">
                <section class="shadow-sm clean-block clean-hero" style="background-image: url(&quot;../assets/img/dia-do-estudante-990x556.jpg&quot;);color: rgba(36,137,13, 0.85);opacity: 1;filter: blur(0px);min-height: 500px;">
                    <div class="text" style="max-width: 832px;">
                     
                        <h1 class="display-4 text-center"><br> Postgraduate Program in Software Engineering (PPGES)&nbsp;<br><br></h1>
                        <p>Postgraduate Program in Software Engineering linked to the Federal University of Pampa (UNIPAMPA)</p>
                    </div>
                </section>
        <section class="shadow-sm clean-block clean-info dark" id="apresentacao" style="padding-bottom: 0px;background-color: transparent;">
            <div class="container">
                <div class="block-heading" style="padding-top: 25px;">
                    <h1 style="border-bottom: 1px solid rgba(0,0,0,.1);padding-bottom: 20px;">Master's Subjects</h1>
                    <div class="table-responsive">
                        <table class="table-bordered" style="width: 100%; margin: auto;">
                            <thead>
                                <tr>
                                    <th>
                                        Code
                                    </th>
                                    <th>
                                        Subject
                                    </th>
                                    <th>
                                        Required
                                    </th>
                                    <th>
                                        Workload
                                    </th>
                                    <th>
                                        Credits
                                    </th>
                                </tr>
                            </thead>
                                <tbody>
                                    <tr style="height: 48px">
                                    <td style="height: 48px;width: 63px">
                                    <p dir="ltr">AL5082</p>
                                    </td>
                                    <td style="height: 48px;width: 338px">
                                    <p dir="ltr">Algorithm Optimization and Complexity</p>
                                    </td>
                                    <td style="height: 48px;width: 89px">
                                    <p dir="ltr">Yes</p>
                                    </td>
                                    <td style="height: 48px;width: 47px">
                                    <p dir="ltr">60</p>
                                    </td>
                                    <td style="height: 48px;width: 41px">
                                    <p dir="ltr">4</p>
                                    </td>
                                    </tr>
                                    <tr style="height: 48px">
                                    <td style="height: 48px;width: 63px">
                                    <p dir="ltr">AL5083</p>
                                    </td>
                                    <td style="height: 48px;width: 338px">
                                    <p dir="ltr">Experimental Software Engineering</p>
                                    </td>
                                    <td style="height: 48px;width: 89px">
                                    <p dir="ltr">Yes</p>
                                    </td>
                                    <td style="height: 48px;width: 47px">
                                    <p dir="ltr">60</p>
                                    </td>
                                    <td style="height: 48px;width: 41px">
                                    <p dir="ltr">4</p>
                                    </td>
                                    </tr>
                                    <tr style="height: 48px">
                                    <td style="height: 48px;width: 63px">
                                    <p dir="ltr">AL5084</p>
                                    </td>
                                    <td style="height: 48px;width: 338px">
                                    <p dir="ltr">Software development</p>
                                    </td>
                                    <td style="height: 48px;width: 89px">
                                    <p dir="ltr">Yes</p>
                                    </td>
                                    <td style="height: 48px;width: 47px">
                                    <p dir="ltr">60</p>
                                    </td>
                                    <td style="height: 48px;width: 41px">
                                    <p dir="ltr">4</p>
                                    </td>
                                    </tr>
                                    <tr style="height: 48px">
                                    <td style="height: 48px;width: 63px">
                                    <p dir="ltr">AL5085</p>
                                    </td>
                                    <td style="height: 48px;width: 338px">
                                    <p dir="ltr">Software Verification and Validation</p>
                                    </td>
                                    <td style="height: 48px;width: 89px">
                                    <p dir="ltr">No</p>
                                    </td>
                                    <td style="height: 48px;width: 47px">
                                    <p dir="ltr">60</p>
                                    </td>
                                    <td style="height: 48px;width: 41px">
                                    <p dir="ltr">4</p>
                                    </td>
                                    </tr>
                                    <tr style="height: 48px">
                                    <td style="height: 48px;width: 63px">
                                    <p dir="ltr">AL5086</p>
                                    </td>
                                    <td style="height: 48px;width: 338px">
                                    <p dir="ltr">Systems Security</p>
                                    </td>
                                    <td style="height: 48px;width: 89px">
                                    <p dir="ltr">No</p>
                                    </td>
                                    <td style="height: 48px;width: 47px">
                                    <p dir="ltr">60</p>
                                    </td>
                                    <td style="height: 48px;width: 41px">
                                    <p dir="ltr">4</p>
                                    </td>
                                    </tr>
                                    <tr style="height: 72px">
                                    <td style="height: 72px;width: 63px">
                                    <p dir="ltr">AL5087</p>
                                    </td>
                                    <td style="height: 72px;width: 338px">
                                    <p dir="ltr">Design and Development of Parallel Programs</p>
                                    </td>
                                    <td style="height: 72px;width: 89px">
                                    <p dir="ltr">No</p>
                                    </td>
                                    <td style="height: 72px;width: 47px">
                                    <p dir="ltr">60</p>
                                    </td>
                                    <td style="height: 72px;width: 41px">
                                    <p dir="ltr">4</p>
                                    </td>
                                    </tr>
                                    <tr style="height: 48px">
                                    <td style="height: 48px;width: 63px">
                                    <p dir="ltr">AL5088</p>
                                    </td>
                                    <td style="height: 48px;width: 338px">
                                    <p dir="ltr">Network Infrastructure Programmability</p>
                                    </td>
                                    <td style="height: 48px;width: 89px">
                                    <p dir="ltr">No</p>
                                    </td>
                                    <td style="height: 48px;width: 47px">
                                    <p dir="ltr">60</p>
                                    </td>
                                    <td style="height: 48px;width: 41px">
                                    <p dir="ltr">4</p>
                                    </td>
                                    </tr>
                                    <tr style="height: 48px">
                                    <td style="height: 48px;width: 63px">
                                    <p dir="ltr">AL5089</p>
                                    </td>
                                    <td style="height: 48px;width: 338px">
                                    <p dir="ltr">Software Modeling</p>
                                    </td>
                                    <td style="height: 48px;width: 89px">
                                    <p dir="ltr">No</p>
                                    </td>
                                    <td style="height: 48px;width: 47px">
                                    <p dir="ltr">60</p>
                                    </td>
                                    <td style="height: 48px;width: 41px">
                                    <p dir="ltr">4</p>
                                    </td>
                                    </tr>
                                    <tr style="height: 48px">
                                    <td style="height: 48px;width: 63px">
                                    <p dir="ltr">AL5090</p>
                                    </td>
                                    <td style="height: 48px;width: 338px">
                                    <p dir="ltr">Agent-Oriented Software Engineering</p>
                                    </td>
                                    <td style="height: 48px;width: 89px">
                                    <p dir="ltr">No</p>
                                    </td>
                                    <td style="height: 48px;width: 47px">
                                    <p dir="ltr">60</p>
                                    </td>
                                    <td style="height: 48px;width: 41px">
                                    <p dir="ltr">4</p>
                                    </td>
                                    </tr>
                                    <tr style="height: 72px">
                                    <td style="height: 72px;width: 63px">
                                    <p dir="ltr">AL5091</p>
                                    </td>
                                    <td style="height: 72px;width: 338px">
                                    <p dir="ltr">Machine Learning Applied to Software Engineering</p>
                                    </td>
                                    <td style="height: 72px;width: 89px">
                                    <p dir="ltr">No</p>
                                    </td>
                                    <td style="height: 72px;width: 47px">
                                    <p dir="ltr">30</p>
                                    </td>
                                    <td style="height: 72px;width: 41px">
                                    <p dir="ltr">2</p>
                                    </td>
                                    </tr>
                                    <tr style="height: 72px">
                                    <td style="height: 72px;width: 63px">
                                    <p dir="ltr">AL5092</p>
                                    </td>
                                    <td style="height: 72px;width: 338px">
                                    <p dir="ltr">Human-Computer Interaction in Software Development</p>
                                    </td>
                                    <td style="height: 72px;width: 89px">
                                    <p dir="ltr">No</p>
                                    </td>
                                    <td style="height: 72px;width: 47px">
                                    <p dir="ltr">60</p>
                                    </td>
                                    <td style="height: 72px;width: 41px">
                                    <p dir="ltr">4</p>
                                    </td>
                                    </tr>
                                    <tr style="height: 72px">
                                    <td style="height: 72px;width: 63px">
                                    <p dir="ltr">AL5093</p>
                                    </td>
                                    <td style="height: 72px;width: 338px">
                                    <p dir="ltr">Natural Language Processing Applied to Software Engineering</p>
                                    </td>
                                    <td style="height: 72px;width: 89px">
                                    <p dir="ltr">No</p>
                                    </td>
                                    <td style="height: 72px;width: 47px">
                                    <p dir="ltr">30</p>
                                    </td>
                                    <td style="height: 72px;width: 41px">
                                    <p dir="ltr">2</p>
                                    </td>
                                    </tr>
                                    <tr style="height: 72px">
                                    <td style="height: 72px;width: 63px">
                                    <p dir="ltr">AL5094</p>
                                    </td>
                                    <td style="height: 72px;width: 338px">
                                    <p dir="ltr">Software Engineering Applied to Embedded Systems</p>
                                    </td>
                                    <td style="height: 72px;width: 89px">
                                    <p dir="ltr">No</p>
                                    </td>
                                    <td style="height: 72px;width: 47px">
                                    <p dir="ltr">60</p>
                                    </td>
                                    <td style="height: 72px;width: 41px">
                                    <p dir="ltr">4</p>
                                    </td>
                                    </tr>
                                    <tr style="height: 48px">
                                    <td style="height: 48px;width: 63px">
                                    <p dir="ltr">AL5013</p>
                                    </td>
                                    <td style="height: 48px;width: 338px">
                                    <p dir="ltr">Supervised Teaching Internship</p>
                                    </td>
                                    <td style="height: 48px;width: 89px">
                                    <p dir="ltr">Yes</p>
                                    </td>
                                    <td style="height: 48px;width: 47px">
                                    <p dir="ltr">15</p>
                                    </td>
                                    <td style="height: 48px;width: 41px">
                                    <p dir="ltr">1</p>
                                    </td>
                                    </tr>
                                    <tr style="height: 48px">
                                    <td style="height: 48px;width: 63px">
                                    <p dir="ltr">AL5038</p>
                                    </td>
                                    <td style="height: 48px;width: 338px">
                                    <p dir="ltr">Foreign Language Proficiency Exam</p>
                                    </td>
                                    <td style="height: 48px;width: 89px">
                                    <p dir="ltr">Yes</p>
                                    </td>
                                    <td style="height: 48px;width: 47px">
                                    <p dir="ltr">–</p>
                                    </td>
                                    <td style="height: 48px;width: 41px"></td>
                                    </tr>
                                    <tr style="height: 48px">
                                    <td style="height: 48px;width: 63px">
                                    <p dir="ltr">AL5039</p>
                                    </td>
                                    <td style="height: 48px;width: 338px">
                                    <p dir="ltr">Qualification Exam</p>
                                    </td>
                                    <td style="height: 48px;width: 89px">
                                    <p dir="ltr">Yes</p>
                                    </td>
                                    <td style="height: 48px;width: 47px">
                                    <p dir="ltr">–</p>
                                    </td>
                                    <td style="height: 48px;width: 41px">
                                    <p dir="ltr">–</p>
                                    </td>
                                    </tr>
                                    <tr style="height: 29px">
                                    <td style="height: 29px;width: 63px">
                                    <p dir="ltr">AL5040</p>
                                    </td>
                                    <td style="height: 29px;width: 338px">
                                    <p dir="ltr">Defesa de Dissertação</p>
                                    </td>
                                    <td style="height: 29px;width: 89px">
                                    <p dir="ltr">Yes</p>
                                    </td>
                                    <td style="height: 29px;width: 47px">
                                    <p dir="ltr">–</p>
                                    </td>
                                    <td style="height: 29px;width: 41px">
                                    <p dir="ltr">–</p>
                                    </td>
                                    </tr>
                                    </tbody>
                            </tbody>
                        </table>
            </div>
        </section>
        
    </main>
    <footer class="page-footer dark" style="padding-top: 0px;">
        <div class="footer-copyright">
            <p><br>© 2014 Universidade Federal do Pampa - UNIPAMPA<br><br></p>
        </div>
    </footer>
    <script src="../assets/js/jquery.min.js"></script>
    <script src="../assets/bootstrap/js/bootstrap.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/baguettebox.js/1.10.0/baguetteBox.min.js"></script>
    <script src="../assets/js/smoothproducts.min.js"></script>
    <script src="../assets/js/theme.js"></script>
    <script src="../myscript.js"></script>
</body>

</html>